...
spool "6_&filename..log"

-- Password file execution
@passworddefinition.sql

PROMPT Dropping Role ROLE_6 ...
DROP ROLE ROLE_6 ;
PROMPT Creating Role ROLE_6 ...
CREATE ROLE ROLE_6 ;

-- PROMPT Drop rent user
-- drop user rent cascade;
   
PROMPT Create user rent
CREATE USER rent IDENTIFIED BY &&rent_password PASSWORD EXPIRE ACCOUNT LOCK /* DEFAULT TABLESPACE USERS  TEMPORARY TABLESPACE TEMP */;
GRANT CREATE SESSION, RESOURCE, CREATE VIEW, CREATE MATERIALIZED VIEW, CREATE SYNONYM, UNLIMITED TABLESPACE TO rent;
...
GRANT ALL ON utils TO ROLE_6;
grant ALL on utils to rent with grant option;

set define on
prompt connecting to rent
alter session set current_schema=rent;
set define off
-- DROP TABLE rents CASCADE CONSTRAINTS;


PROMPT Creating Table rents ...
CREATE TABLE rents (
  rent_id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  apartment_id NUMBER(10,0),
  status NVARCHAR2(50),
  date_begin VARCHAR2(200 CHAR),
  date_end VARCHAR2(200 CHAR),
  client_passport_number NVARCHAR2(50),
  type NVARCHAR2(50)
);


PROMPT Creating Primary Key Constraint PK__rents__03FC6F3534F69D1B on table rents ... 
ALTER TABLE rents
ADD CONSTRAINT PK__rents__03FC6F3534F69D1B PRIMARY KEY
(
  rent_id
)
ENABLE
...
GRANT ALL ON rents TO ROLE_6;
-- DROP TABLE apartments CASCADE CONSTRAINTS;


PROMPT Creating Table apartments ...
CREATE TABLE apartments (
  apartment_id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  city NVARCHAR2(50),
  street NVARCHAR2(50),
  house_number NUMBER(10,0),
  room_number NUMBER(10,0),
  day_cost NUMBER(19,4),
  location RAW(8000)
);


PROMPT Creating Primary Key Constraint PK__apartmen__DC51C2ECD77AC9D9 on table apartments ... 
ALTER TABLE apartments
ADD CONSTRAINT PK__apartmen__DC51C2ECD77AC9D9 PRIMARY KEY
(
  apartment_id
)
ENABLE
...
GRANT ALL ON apartments TO ROLE_6;
-- DROP TABLE bill_details CASCADE CONSTRAINTS;


PROMPT Creating Table bill_details ...
CREATE TABLE bill_details (
  bill_id NUMBER(10,0) GENERATED BY DEFAULT ON NULL AS IDENTITY START WITH 1 INCREMENT BY 1 MINVALUE 1 NOMAXVALUE ,
  rent_id NUMBER(10,0),
  bill_date VARCHAR2(200 CHAR),
  total NUMBER(19,4)
);


PROMPT Creating Primary Key Constraint PK__bill_det__D706DDB31903B6ED on table bill_details ... 
ALTER TABLE bill_details
ADD CONSTRAINT PK__bill_det__D706DDB31903B6ED PRIMARY KEY
(
  bill_id
)
ENABLE
...
GRANT ALL ON bill_details TO ROLE_6;
...
CREATE TABLE clients (
  ...
  name NVARCHAR2(50),
  passport_number NVARCHAR2(50) NOT NULL,
  hid RAW(8000)
);


PROMPT Creating Primary Key Constraint PK__clients__D2CA6298A0879233 on table clients ... 
ALTER TABLE clients
ADD CONSTRAINT PK__clients__D2CA6298A0879233 PRIMARY KEY
(
  passport_number
)
ENABLE
...
GRANT ALL ON clients TO ROLE_6;
PROMPT Creating Index date_index on rents ...
CREATE INDEX date_index ON rents
(
  date_begin
) 
;
PROMPT Creating Index apartment_index on apartments ...
CREATE INDEX apartment_index ON apartments
(
  city,
  street,
  house_number,
...
;
PROMPT Creating Index MySpatialIndex on apartments ...
CREATE INDEX MySpatialIndex ON apartments
(
  location
) 
;
PROMPT Creating Index rent_index on bill_details ...
CREATE INDEX rent_index ON bill_details
(
  rent_id,
  bill_date
) 
...
alter session set current_schema=rent;
set define off
CREATE OR REPLACE FORCE VIEW rents_with_apartments 
AS 
   SELECT client_passport_number ,
          ...
          date_end 
     FROM rents 
            JOIN apartments    ON rents.apartment_id = apartments.apartment_id;


GRANT ALL ON rents_with_apartments TO ROLE_6;
CREATE OR REPLACE FORCE VIEW clients_with_bills 
AS 
   SELECT bill_date ,
          total ,
          STATUS ,
          ...
          passport_number 
     FROM bill_details 
            LEFT JOIN rents    ON bill_details.rent_id = rents.rent_id
            LEFT JOIN clients    ON clients.passport_number = rents.client_passport_number;


GRANT ALL ON clients_with_bills TO ROLE_6;
CREATE OR REPLACE FORCE VIEW bills_with_apartments 
AS 
   SELECT bill_id ,
          bill_date ,
          total ,
...
GRANT ALL ON bills_with_apartments TO ROLE_6;
CREATE OR REPLACE FUNCTION client_rent_amount
(
  v_client_passport_number IN NVARCHAR2
)
RETURN NUMBER
AS
...
BEGIN
   SELECT COUNT(*)  
     ...
     INTO v_result
     FROM rents 
            LEFT JOIN bill_details    ON bill_details.rent_id = rents.rent_id
    WHERE  v_client_passport_number = client_passport_number;
   RETURN v_result;

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON client_rent_amount TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.fn_diagramobjects
RETURN NUMBER
AUTHID DEFINER
AS
...
END;
/
GRANT ALL ON fn_diagramobjects TO ROLE_6;
CREATE OR REPLACE FUNCTION rent_days_amount
(
  v_rent_id IN NUMBER
)
RETURN NUMBER
AS
   v_date_begin VARCHAR2(200);
   v_date_end VARCHAR2(200);
...
BEGIN
   ...
   SELECT date_end 
     ...
     FROM rents 
    WHERE  rent_id = v_rent_id;
   RETURN utils.datediff('DAY', v_date_begin, v_date_end);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON rent_days_amount TO ROLE_6;
CREATE OR REPLACE FUNCTION total_money
RETURN NUMBER
AS
   v_result NUMBER(19,4);
...
END;
/
GRANT ALL ON total_money TO ROLE_6;
CREATE OR REPLACE PROCEDURE AddChildNode
(
  v_parent_passport_number IN NVARCHAR2,
  v_passport_number IN NVARCHAR2,
  v_surname IN NVARCHAR2,
  v_name IN NVARCHAR2
)
AS
   v_parenthid UDT.HIERARCHYID%TYPE;
   v_hid UDT.HIERARCHYID%TYPE;
...
BEGIN
   ...
   SELECT hid 
     ...
     INTO v_parenthid
     FROM clients 
    WHERE  passport_number = v_parent_passport_number;
   SELECT MAX(hid)  

     INTO v_hid
     FROM clients 
    WHERE  GetAncestor(1) = v_parenthid;clients VALUES ( @surname , @name , @passport_number , @parenthid . GetDescendant ( @hid , NULL ) )  --SQLDEV: NOT RECOGNIZED

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON AddChildNode TO ROLE_6;
CREATE OR REPLACE PROCEDURE GetChildNodes
(
  v_passport_number IN NVARCHAR2
)
AS
   v_parentNodeHid UDT.HIERARCHYID%TYPE;
...
BEGIN
   ...
   SELECT ( SELECT hid 
            FROM clients 
             WHERE  passport_number = v_passport_number ) 

     INTO v_parentNodeHid
     FROM DUAL ;
   OPEN  v_cursor FOR
      SELECT ToString() hid  ,
             GetLevel() LEVEL_  ,
             * 
        FROM clients 
       WHERE  GetAncestor(1) = v_parentNodeHid ;
      DBMS_SQL.RETURN_RESULT(v_cursor);

EXCEPTION WHEN OTHERS THEN utils.handleerror(SQLCODE,SQLERRM);
END;
/
GRANT ALL ON GetChildNodes TO ROLE_6;
CREATE OR REPLACE PROCEDURE pay_for_apartment
(
  v_rent_id IN NUMBER
)
AS
...
BEGIN
   ...
   IF v_temp = 1 THEN
    DECLARE
      v_total NUMBER(19,4);
      v_per_day NUMBER(19,4);
      v_days NUMBER(10,0);
      v_date VARCHAR2(200);
   
   BEGIN
      v_days := rent_days_amount(v_rent_id) ;
      SELECT day_cost 

        INTO v_per_day
        FROM apartments 
               JOIN rents    ON apartments.apartment_id = rents.apartment_id
       WHERE  rents.rent_id = v_rent_id 
        FETCH FIRST 1 ROWS ONLY;
      v_total := v_days * v_per_day ;
      v_date := UTILS.CONVERT_TO_VARCHAR2(SYSDATE,200) ;
      INSERT INTO bill_details
        VALUES ( DEFAULT, v_rent_id, v_date, v_total );
...
END;
/
GRANT ALL ON pay_for_apartment TO ROLE_6;
CREATE OR REPLACE PROCEDURE register_apartment
(
  v_city IN NVARCHAR2,
  v_street IN NVARCHAR2,
  v_house_number IN NUMBER,
  v_room_number IN NUMBER,
  v_day_cost IN NUMBER
)
AS
   v_temp NUMBER(1, 0) := 0;
...
BEGIN
      ...
      SELECT 1 INTO v_temp
       ...
       WHERE ( NOT EXISTS ( SELECT * 
                            FROM apartments 
                             WHERE  v_city = city
                                      AND v_street = street
                                      AND v_house_number = house_number
                                      AND v_room_number = room_number ) );
   EXCEPTION
      WHEN OTHERS THEN
         NULL;
   END;
   ...
   BEGIN
      INSERT INTO apartments
        VALUES ( DEFAULT, v_city, v_street, v_house_number, v_room_number, v_day_cost );
...
END;
/
GRANT ALL ON register_apartment TO ROLE_6;
...
BEGIN
   ...
   BEGIN
      INSERT INTO clients
        VALUES ( v_surname, v_name, v_passport_number );
...
END;
/
GRANT ALL ON register_client TO ROLE_6;
CREATE OR REPLACE PROCEDURE rent_apartment
(
  v_apartment_id IN NUMBER,
  v_date_begin IN VARCHAR2,
...
BEGIN
   ...
   IF ( utils.datediff('DAY', v_date_end, v_date_begin) < 0 ) THEN
    DECLARE
      v_temp NUMBER(1, 0) := 0;
   ...
   BEGIN
      BEGIN
         SELECT 1 INTO v_temp
          ...
          WHERE ( NOT EXISTS ( SELECT * 
                               FROM rents 
                                WHERE  date_begin < v_date_end
                                         AND date_end > v_date_begin
      ...
      BEGIN
         INSERT INTO rents
           VALUES ( DEFAULT, v_apartment_id, 'not paid', v_date_begin, v_date_end, v_client_passport_number, v_type );
      
      END;
      ELSE
      
      BEGIN
         DBMS_OUTPUT.PUT_LINE(('cannot rent'));
...
END;
/
GRANT ALL ON rent_apartment TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.sp_alterdiagram
(
...
END;
/
GRANT ALL ON sp_alterdiagram TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.sp_creatediagram
(
  v_diagramname IN VARCHAR2,
  iv_owner_id IN NUMBER DEFAULT NULL ,
  v_version IN NUMBER,
...
END;
/
GRANT ALL ON sp_creatediagram TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.sp_dropdiagram
(
  v_diagramname IN VARCHAR2,
  iv_owner_id IN NUMBER DEFAULT NULL 
)
...
END;
/
GRANT ALL ON sp_dropdiagram TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.sp_helpdiagramdefinition
(
  v_diagramname IN VARCHAR2,
  iv_owner_id IN NUMBER DEFAULT NULL 
)
...
END;
/
GRANT ALL ON sp_helpdiagramdefinition TO ROLE_6;
CREATE OR REPLACE PROCEDURE rent.sp_helpdiagrams
(
...
END;
/
GRANT ALL ON sp_helpdiagrams TO ROLE_6;
CREATE OR REPLACE FUNCTION rent.sp_renamediagram
(
  v_diagramname IN VARCHAR2,
  iv_owner_id IN NUMBER DEFAULT NULL ,
...
END;
/
GRANT ALL ON sp_renamediagram TO ROLE_6;
/* Translation Extracted DDL For Required Objects*/
CREATE TABLE rent.sysdiagrams
(
...
/
CREATE SEQUENCE tt_rent.sysdiagrams_diagram 
;
/
CREATE OR REPLACE TRIGGER 
 tt_TR_t.sysdiagrams_diagram    BEFORE INSERT 
   ON rent.sysdiagrams
   FOR EACH ROW
   BEGIN
      SELECT tt_rent.sysdiagrams_diagram.NEXTVAL INTO :NEW.diagram_id
        FROM DUAL;
   END;
/


CREATE OR REPLACE FUNCTION rent.sp_upgraddiagrams
RETURN NUMBER
AS
   ...
   BEGIN
      INSERT INTO rent.sysdiagrams
        ...
        ( SELECT UTILS.CONVERT_TO_VARCHAR2(dgnm.uvalue,128) ,
                 ...
                 dgdef.lvalue 
          FROM rent.dtproperties dgnm
                 JOIN rent.dtproperties dggd   ON dggd.property = 'DtgSchemaGUID'
                 AND dggd.objectid = dgnm.objectid
                 JOIN rent.dtproperties dgdef   ON dgdef.property = 'DtgSchemaDATA'
...
END;
/
GRANT ALL ON sp_upgraddiagrams TO ROLE_6;
CREATE OR REPLACE TRIGGER rents_delete
   AFTER DELETE
   ON rents
   FOR EACH ROW

BEGIN
   DBMS_OUTPUT.PUT_LINE('rent was deleted');
...
BEGIN
   MERGE INTO rents 
   USING (SELECT rents.ROWID row_id, 'not paid' AS STATUS
   FROM rents 
          JOIN DELETED    ON rents.rent_id = :OLD.rent_id ) src
   ON ( rents.ROWID = src.row_id )
...
BEGIN
   SELECT :NEW.rent_id 
     ...
     FROM DUAL ;
   MERGE INTO rents 
   USING (SELECT rents.ROWID row_id, 'paid'
   FROM rents 
          JOIN INSERTED    ON rents.rent_id = :NEW.rent_id ) src
   ON ( rents.ROWID = src.row_id )
...
alter session set current_schema=rent;
set define off
PROMPT Creating Foreign Key Constraint FK__rents__apartment__286302EC on table apartments...
ALTER TABLE rents
ADD CONSTRAINT FK__rents__apartment__286302EC FOREIGN KEY
(
  apartment_id
)
...
PROMPT Creating Foreign Key Constraint FK__rents__client_pa__29572725 on table clients...
ALTER TABLE rents
ADD CONSTRAINT FK__rents__client_pa__29572725 FOREIGN KEY
(
...
PROMPT Creating Foreign Key Constraint FK__bill_deta__rent___2C3393D0 on table rents...
ALTER TABLE bill_details
ADD CONSTRAINT FK__bill_deta__rent___2C3393D0 FOREIGN KEY
(
  rent_id
)
REFERENCES rents
...
ALTER VIEW rents_with_apartments COMPILE;

ALTER VIEW clients_with_bills COMPILE;

ALTER VIEW bills_with_apartments COMPILE;
